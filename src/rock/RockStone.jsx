...
export default function RockStone(props) {
  ...
  // Center pivot: move model so that pivotRef's origin becomes model's bbox center
  useLayoutEffect(() => {
    ...
    // World-space bounding box center of the model
    const box = new THREE.Box3().setFromObject(modelRef.current)
    const worldCenter = new THREE.Vector3()
    box.getCenter(worldCenter)

    // Convert that center into pivot (parent) local space
