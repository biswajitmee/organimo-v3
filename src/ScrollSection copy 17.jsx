...
function Scene () {
  ...
  const curveGroupRef = useRef() // theatre editable group reference
  const sphereRef = useRef()
  const wrapperRef = useRef()
  const curveRef = useRef()
  const phaseRef = useRef(null) // safer phase tracker
  ...
  function computeSpringCamDesired (brickT, groupMatrix, preferOpposite = false) {
    // same existing brick->world calculation but added option to place camera on opposite side
    ...
    // compute default camera offset (as before)
    ...
    // compute quaternion facing the brick (like before) but ensure direction consistent
  ...
  useFrame((state, delta) => {
    ...
    // --- compute curve camera (world) ---
    const curveCamPos = new THREE.Vector3()
    const curveLookAt = new THREE.Vector3()
    if (curveRef.current && typeof curveRef.current.getPoint === 'function') {
      curveRef.current.getPoint(curveT, curveCamPos)
      const aheadT = curveT > 0.98 ? 1.0 : THREE.MathUtils.clamp(curveT + curveAhead, 0, 1)
      curveRef.current.getPoint(aheadT, curveLookAt)
    } else {
      curveCamPos.copy(fallbackHelix.getPoint(curveT).multiplyScalar(pathScale))
      curveLookAt.copy(fallbackHelix.getPoint(THREE.MathUtils.clamp(curveT + curveAhead, 0, 1)).multiplyScalar(pathScale))
    }

    // --- compute spring camera candidate ---
    ...
    // reverse path start: start from opposite end
    const brickT_forSpring = 1.0 - (brickIndex + 0.5) / count

    const groupMatrix = ensureMatrixWorld()

    // For transition, prefer opposite side placement so camera starts opposite the actual spring brick
    ...
    // decide blend factor around split
    const lo = split - TRANSITION_EPS
    const hi = split + TRANSITION_EPS
    const alphaRaw = smoothstep(lo, hi, tRaw)
    const alpha = easeOutCubic(alphaRaw) // 0 = curve, 1 = spring

    // ---------------- CURVE PHASE: clamp pitch + smooth ----------------
    if (inCurvePhase) {
      // position smoothing (so position doesn't snap)
      const posSmooth = THREE.MathUtils.clamp(1 - Math.exp(-positionSmoothing * 10 * delta), 0, 1)
      cameraRef.current.position.lerp(curveCamPos, posSmooth)

      // compute lookAt quaternion, then clamp X (pitch) to +/- 30 degrees and slerp toward it
      const m = new THREE.Matrix4()
      m.lookAt(cameraRef.current.position, curveLookAt, new THREE.Vector3(0, 1, 0))
      const targetQuat = new THREE.Quaternion().setFromRotationMatrix(m)

      // convert to euler to clamp pitch (X) — use YXZ or YXZ? we use 'YXZ' consistent with other code
      const euler = new THREE.Euler().setFromQuaternion(targetQuat, 'YXZ')

      // clamp pitch to +/- 30 degrees (convert to radians)
      const maxCurvePitchRad = THREE.MathUtils.degToRad(30)
      euler.x = THREE.MathUtils.clamp(euler.x, -maxCurvePitchRad, maxCurvePitchRad)

      // convert back to quaternion
      const clampedQuat = new THREE.Quaternion().setFromEuler(euler)

      // rotation smoothing (use smaller factor for curve so it moves slowly)
      ...
      // hide debug marker while on curve
      ...
      // Blend from opposite starter to actual spring-target with alpha
      ...
      // show debug sphere at real brick position
      ...
      // bricks activation logic (unchanged)
      const t = springT
  ...
  return (
    <>
      ...
      <group ref={wrapperRef}>
        {/* Theatre-editable wrapper around CurvePath */}
