...
function Scene () {
  ...
  useFrame((state, delta) => {
    ...
    if (isOverriding && cameraRef.current) {
      ...
      console.log(`[ACTIVE PATH] springPath camera active — scroll.offset= ${dbgOffset} brickIndex ~ ${brickIndex}`)
    }
    ...
    <>
      ...
      {isOverriding ? (
        <perspectiveCamera
          ...
          far={5000}
          fov={35}
          // initial position is set/snapped on override enter, keep default so r3f mounts it
          position={[0, 2, 10]}
        />
      ) : (
        <PerspectiveCamera
      ...
      <group ref={wrapperRef}>
        ...
        <directionalLight position={[30, 40, 10]} intensity={0.25} castShadow={true} shadow-mapSize-width={1024} shadow-mapSize-height={1024} shadow-camera-near={0.5} shadow-camera-far={200} shadow-camera-left={-50} shadow-camera-right={50} shadow-camera-top={50} shadow-camera-bottom={-50} />
        ...
        <e.group theatreKey='Pillarstone' position={[0, 0, -1]}>
          <Float speed={1.5} rotationIntensity={0.1} floatIntensity={0.5} floatingRange={[-2, 2]}><Pillarstone scale={10} /></Float>
        </e.group>
