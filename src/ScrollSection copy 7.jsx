...
scrollSpeedMultiplier: { value: 1.0, min: 0.01, max: 1.0, step: 0.01, label: 'Scroll Speed Multiplier' },
  ...
  const snapRef = useRef({
    active: false,
    start: 0,
    duration: (typeof guiFadeDefaults.snapDurationSec === 'number' ? guiFadeDefaults.snapDurationSec : 2) * 1000,
  ...
  useFrame(() => {
    ...
    const directionDown = currScrollOffset == null ? true : currScrollOffset > (lastRawRef.current || 0)
...
try {
  ...
  useFrame((state, delta) => {
    ...
    const extraPitchDeg = 5 * progress   // 0 -> 10 degrees
camEuler.x += THREE.MathUtils.degToRad(extraPitchDeg)
  ...
  window._springSuppressTheatreResume = true
        ...
        // NEW SAFETY: ensure theatre resumes when forcedBlend finishes (fallback)
        try {
          ...
          if (ctrl && ctrl.sessionId && !(ctrl.exit || ctrl.exited)) {
            if (ctrl.enter) {
              ...
              if (exitAllowed) {
                ...
                // NEW SAFETY: when stability-based exit occurs, resume theatre immediately
