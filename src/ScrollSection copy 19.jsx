...
function FrontCopy({
  ...
  useFrame(() => {
    ...
    for (let i = 0; i < seg; i++) {
      ...
      const localPoint = curve.getPoint(tt, new THREE.Vector3())
...
function Scene ({ overlayRefHolder }) {
  ...
  /* ------------------ Separate GUIs ------------------ */
  // SPRING controls: root (not under a named folder) -> keeps Spring settings independent
  const springControls = useControls({
    // spring path geometry & scale (these are the original names used across the code)
  ...
  // CURVE controls: put in a named folder/group 'CurvePath' (expandable)
  const curveControls = useControls('CurvePath', {
    ...
    // camera local offset & extra rotation (curve only)
  ...
  }, { collapsed: true }) // start collapsed; user can expand

  // small misc group for transition fade
  ...
  // brick spec defaults (curve & spring may override via props)
  ...
  useFrame((state, delta) => {
    ...
    // Curve pose (use CurvePath component API if available)
    let poseLocal = null
    ...
    // Spring camera candidates (spring-only values)
    ...
    if (inCurvePhase) {
      ...
      // apply small extra offsets from curveControls? The curve camera orientation comes from curveTargetQuat (clamped)
  ...
  // update overlayDOMRef and fade refs
  useEffect(() => {
    if (overlayRefHolder && overlayRefHolder.current) overlayDOMRef.current = overlayRefHolder.current
  }, [overlayRefHolder])
