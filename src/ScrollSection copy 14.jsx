...
function Scene () {
  ...
  const controls = useControls({
    ...
    // preview (camera-front) controls
    previewVisible: { value: true },
    previewDistance: { value: 2.0, min: 0.2, max: 12, step: 0.1 }, // <-- smaller = closer to camera
    previewScale: { value: 0.6, min: 0.02, max: 4, step: 0.01 }, // <-- larger = bigger path
    previewScaleZ: { value: 0.6, min: 0.02, max: 4, step: 0.01 }, // optional separate Z-scale
    previewHelixColor: { value: '#1e90ff' },
    previewCurveColor: { value: '#ff3b30' },
    previewSegments: { value: 240, min: 16, max: 1024, step: 1 }
  })

  const {
    turns, coilRadius, pathHeight, pathScale,
    previewVisible, previewDistance, previewScale, previewScaleZ,
    previewHelixColor, previewCurveColor, previewSegments
  } = controls

  /* fallback spring path params (kept) */
  const {
    brickCount = 25,
    radialOffset = 0,
    cameraSideOffset = -10,
    ...
    minCamY = -5,
    maxCamY = 50,
    maxMovePerFrameFactor = 1,
    mode = 'oppositeSideMove',
    startAt = 'top'
  } = controls

  /* ---------------- preview geometries ---------------- */
  const previewHelixGeometry = useMemo(() => {
    const curve = new HelixCurve({ turns: Math.max(0.1, turns), radius: coilRadius * 0.18, height: pathHeight * 0.08 })
    const pts = []
    const segs = Math.max(8, Math.floor(previewSegments || 240))
    ...
    return geom
  }, [turns, coilRadius, pathHeight, previewSegments])

  const previewArcGeometry = useMemo(() => {
    const pts = []
    const segs = 64
    const radius = Math.max(0.5, Math.min(3.0, coilRadius * 0.12))
    for (let i = 0; i <= segs; i++) {
      const a = (i / segs) * Math.PI * 0.8 - Math.PI * 0.4
      const x = Math.cos(a) * radius * 0.6
      const y = Math.sin(a) * radius * 0.2
      const z = -radius * 0.8
      pts.push(x, y, z)
    }
    const geom = new THREE.BufferGeometry()
  ...
  useFrame((state, delta) => {
    ...
    // previewGroup: copy camera transform and offset forward
    if (previewGroupRef.current && cameraRef.current) {
      previewGroupRef.current.position.copy(cameraRef.current.position)
      previewGroupRef.current.quaternion.copy(cameraRef.current.quaternion)
      previewGroupRef.current.updateMatrixWorld()
      const forward = new THREE.Vector3(0, 0, -previewDistance)
      forward.applyQuaternion(cameraRef.current.quaternion)
      previewGroupRef.current.position.copy(cameraRef.current.position).add(forward)
      // apply separate Z-scale if requested (scaleXYZ)
      previewGroupRef.current.scale.set(previewScale, previewScale, previewScaleZ)
    }

    // rest of camera-follow logic (unchanged)
    ...
    const scaleLerp = 1 - Math.min(1, (absDiff * startupBias) / Math.max(1, count * 0.25))
    let lerpFactor = baseLerp * (0.2 + 0.8 * scaleLerp)
  ...
  return (
    <>
      ...
      <group ref={wrapperRef}>
        ...
        <directionalLight position={[30, 40, 10]} intensity={0.25} castShadow shadow-mapSize-width={1024} shadow-mapSize-height={1024} />
        ...
        <e.group theatreKey='L1stone' position={[0, 0, -1]}><Float speed={2} rotationIntensity={0.1} floatIntensity={0.7} floatingRange={[-2,2]}><L1stone scale={10} /></Float></e.group>
        <e.group theatreKey='L2stone' position={[0, 0, -1]}><Float speed={5} rotationIntensity={0.1} floatIntensity={0.7} floatingRange={[-2,2]}><L2stone scale={10} /></Float></e.group>
        <e.group theatreKey='L3stone' position={[0, 0, -1]}><L3stone /></e.group>
        <e.group theatreKey='R1stone' position={[0, 0, -1]}><R1stone /></e.group>
        <e.group theatreKey='Pillarstone' position={[0, 0, -1]}><Float speed={1.5} rotationIntensity={0.1} floatIntensity={0.5} floatingRange={[-2,2]}><Pillarstone scale={10} /></Float></e.group>
      ...
      {/* preview group always placed in front of camera (position updated in useFrame) */}
      <group ref={previewGroupRef} visible={previewVisible} >
        <line geometry={previewHelixGeometry}><lineBasicMaterial color={previewHelixColor} linewidth={2} /></line>
        <line geometry={previewArcGeometry}><lineBasicMaterial color={previewCurveColor} linewidth={2} /></line>
      </group>
    </>
  )
}
