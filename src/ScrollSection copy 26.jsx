...
function CameraDebugGUI ({ cameraRef, isOverriding }) {
  useControls(
    'Camera Debug',
    {
      ...
      // Euler from quaternion using YXZ order (we use 'YXZ' in scene)
...
export default function ScrollSection () {
  ...
  return (
    <div style={{ height: '100vh', overflow: 'hidden' }}>
      ...
      {/* DOM overlay for copy (outside Canvas) */}
      {/* Note: Scene component will pass cameraRef into overlay via global store? Simpler: mount overlay inside Scene via portal.
          But to keep this file self-contained, overlay is rendered here and Scene will set window._springCamRef for it to read.
          (A small hack but non-invasive). */}
      <CameraOverlayBridge />
    </div>
  )
}

/* ---------------- Bridge: the Scene will set window._springCamRef = cameraRef so overlay can read it.
   This keeps overlay outside Canvas (so it won't cause r3f errors). ---------------- */
...
function Scene () {
  ...
  // expose cameraRef to overlay via global so overlay (outside Canvas) can read it safely
  ...
  useFrame(() => {
    ...
    if (shouldOverride !== prevOverrideRef.current) {
      if (shouldOverride) {
        ...
        // snap camera to path transform immediately
        try {
        ...
        const fallbackPos = cameraRef.current ? cameraRef.current.position.clone() : new THREE.Vector3(0, 2, 10)
        const fallbackQuat = cameraRef.current ? cameraRef.current.quaternion.clone() : new THREE.Quaternion()
