...
function Scene () {
  ...
  useFrame((state, delta) => {
    ...
    // const brickT_forSpring = (brickIndex + 0.5) / count
    ...
    // --- If IN CURVE PHASE: lock camera to curve (position + lookAt) ---
    if (inCurvePhase) {
      cameraRef.current.position.copy(curveCamPos)
      // immediate lookAt curveLookAt
      cameraRef.current.lookAt(curveLookAt)
      // apply small user offsets via quaternion if needed
      // (we won't slerp here; curve-phase is strict)
      ...
      // --- SPRING PHASE ---
      // We want camera to start from the opposite side (springCandidateOpp) and look at the path/bricks.
      // Then as alpha moves to 1 we blend to full spring behaviour.
